...
import { Loader2, Calendar, FileText, Download, Building, DollarSign, Package, ShoppingCart } from "lucide-react";
...
export function OrdersPage() {
    ...
    const StatusBadge = ({ status }: { status: string }) => {
        ...
        return (
            <Badge variant="outline" className={`font-mono ${variants[status] || variants['DRAFT']}`}>
                {status}
            </Badge>
        );
    };
                ...
                <div className="flex items-center gap-4 bg-white/50 dark:bg-black/50 px-4 py-2 rounded-full border border-white/20 shadow-sm backdrop-blur-md">
                    <div className="flex flex-col items-end">
                        ...
                        <span className="text-lg font-bold text-foreground">
                            ${orders.reduce((acc, curr) => acc + (curr.amount || 0), 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div className="max-w-6xl mx-auto space-y-4">
                    {isLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center p-32 text-muted-foreground glass border border-white/10 rounded-2xl bg-black/5">
                            ...
                            <p className="max-w-sm text-center">No purchase orders found in the database. Use the autonomous chat interface to spawn new procurement tasks.</p>
                        </div>
                    ) : (
                            ...
                            {orders.map((order) => (
                                ...
                                >
                                    ...
                                    <div className={`absolute left-0 top-0 bottom-0 w-1 ${
                                        order.status === 'COMPLETED' ? 'bg-green-500/80 shadow-[0_0_10px_rgba(34,197,94,0.5)]' :
                                        order.status === 'PENDING' ? 'bg-yellow-500/80 shadow-[0_0_10px_rgba(234,179,8,0.5)]' :
                                        order.status === 'DRAFT' ? 'bg-gray-400/80' :
                                        'bg-red-500/80'
                                    }`} />
                                        ...
                                        <div className="w-full md:w-auto flex md:flex-col items-center justify-end gap-3 pt-4 md:pt-0 border-t md:border-t-0 border-black/5 dark:border-white/10 shrink-0 self-stretch md:pl-6">
                                            {order.pdf_path ? (
                                                <Button 
                                                    ...
                                                    asChild
                                                >
                                                    <a href={`http://localhost:8000/media/${order.pdf_path.split('/').pop()}`} target="_blank" rel="noopener noreferrer">
